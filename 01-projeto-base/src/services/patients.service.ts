...
type PatientWithCountRow = PatientRow & { encounter_count: number };
...
function toPatientWithCountJson(row: PatientWithCountRow) {
  return { ...toPatientJson(row), encounterCount: row.encounter_count };
}
...
export const patientsService = {
  list() {
    const rows = db
      .prepare(
        `SELECT p.id, p.name, p.birth_date, p.national_id, p.active, p.photo_path,
      ...
      .all() as PatientWithCountRow[];

    return rows.map(toPatientWithCountJson);
  },
