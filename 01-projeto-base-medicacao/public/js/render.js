...
export function renderDetail(state, container){
  const selectedMedication = state.selectedMedication;
  ...
  const { patientName, medicationName, dosage, route, scheduledAt, notes } = selectedMedication;
  
  container.hidden = false;
  container.innerHTML = `
    <h2>${escapeHtml(medicationName)} — ${escapeHtml(dosage)}</h2>
    <p><strong>Paciente:</strong> ${escapeHtml(patientName)}</p>
    <p><strong>Via:</strong> ${escapeHtml(route)}</p>
    <p><strong>Horário:</strong> ${formatDateTime(scheduledAt)}</p>
    ${notes ? `<p><strong>Observações:</strong> ${escapeHtml(notes)}</p>` : ""}
    <button id="remove-button">Suspender</button>
  `;
}